...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("Enter the inital temperature of the water, in Celcius");
	    ...
	    String initialPhase = "liquid";//liquid is the default phase, but it can be changed depending on temperature.
	    if (initialTemp < 0)
	        initialPhase = "solid";
        if (initialTemp > 100)
            initialPhase = "vapor";
            
        String finalPhase = "liquid";//liquid is the default phase, but it can be changed depending on temperature.
	    if (finalTemp < 0)
	        finalPhase = "solid";
        if (finalTemp > 100)
            finalPhase = "vapor";
	    ...
	    if(initialPhase.equals("solid")) {
            ...
            if(!finalPhase.equals("solid")) {
                ...
                heatEnergy += tempChangeLiquid(mass, 0, finalTemp, finalPhase, endothermic); //startTemp = 002108  
                
            }
            
            if(finalPhase.equals("vapor")) {
                heatEnergy += vaporization(mass, endothermic);
                heatEnergy += tempChangeVapor(mass, 100, finalTemp, finalPhase, endothermic); //startTemp = 002108  
        ...
        //initial phase: liquid
	        ...
	        if(initialPhase.equals("vapor")) {
                ...
                if(!finalPhase.equals("vapor")) {
                    heatEnergy += vaporization(mass, endothermic);
                    heatEnergy += tempChangeLiquid(mass, 100, finalTemp, finalPhase, endothermic); //startTemp = 002108  
                
                }
            
                if(finalPhase.equals("sollid")) {
                    heatEnergy += fusion(mass, endothermic);
                    heatEnergy += tempChangeSolid(mass, 0, finalTemp, finalPhase, endothermic); //startTemp = 002108  

                }
            
	        }//end of section for initial phase: vapor

        System.out.println("Totoal Heat Energy: " + heatEnergy + "kJ");
